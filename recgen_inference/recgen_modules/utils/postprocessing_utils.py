"""GLB post-processing with texture baking.

This module requires heavy optional dependencies:
    nvdiffrast, xatlas, pyvista, pymeshfix, igraph, utils3d, opencv-python
...
try:
    ...
    RAST_BACKEND = os.environ.get('RAST_BACKEND', 'cuda')
...
def _fill_holes(
    ...
    rastctx = utils3d.torch.RastContext(backend=RAST_BACKEND)
...
def to_glb(
    ...
    Requires optional deps: nvdiffrast, xatlas, pyvista, pymeshfix, igraph, utils3d.
    Install with: pip install recgen[glb]
    ...
    Args:
        ...
        verbose (bool): Whether to print progress.
    """
    if not _HAS_GLB_DEPS:
        raise ImportError(
            f"GLB export requires optional dependencies (nvdiffrast, xatlas, pyvista, pymeshfix, igraph, utils3d). "
            f"Install with: pip install recgen[glb]\n"
            f"Original error: {_GLB_IMPORT_ERROR}"
        )
    ...
    try:
        ...
        if 'out of memory' in error_msg:
            print('Warning: Texture baking optimization OOM; falling back to fast rasterization.')
            ...
            try:
                ...
                fast_msg = str(fast_exc).lower()
                if 'out of memory' in fast_msg:
                    print('Warning: Texture baking fast mode OOM; using flat fallback texture.')
                    texture = np.full((texture_size, texture_size, 3), 255, dtype=np.uint8)
                else:
                    raise
        else:
            raise
