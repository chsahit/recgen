...
class Gaussian:
    ...
    def save_ply(self, path, transform=[[1, 0, 0], [0, 0, -1], [0, 1, 0]]):
        ...
        Args:
            ...
            transform: Optional 3x3 rotation matrix or 4x4 affine matrix.
                If 3x3, applies a pure rotation to positions and quaternions.
                If 4x4, decomposes into rotation, uniform scale, and translation,
                applying each to positions, scales, and quaternions respectively.
                Default applies a Y-Z swap (Z-up to Y-up).
                Pass None to save in the model's internal coordinate frame.
        """
        ...
        if transform is not None:
            ...
            if transform.shape == (4, 4):
                # Decompose 4x4 affine: rotation (with scale) + translation
                rot_scale = transform[:3, :3]
                translation = transform[:3, 3]
                # Extract uniform scale from the rotation-scale matrix
                sx = np.linalg.norm(rot_scale[:, 0])
                sy = np.linalg.norm(rot_scale[:, 1])
                sz = np.linalg.norm(rot_scale[:, 2])
                uniform_scale = (sx + sy + sz) / 3.0
                # Pure rotation matrix (scale removed)
                rot_matrix = rot_scale / np.array([sx, sy, sz])[None, :]

                # Transform positions: rotate+scale then translate
                xyz = np.matmul(xyz, rot_scale.T) + translation
                # Transform Gaussian scales (log-space): add log of uniform scale
                scale = scale + np.log(uniform_scale)
                # Transform quaternion rotations
                rotation = utils3d.numpy.quaternion_to_matrix(rotation)
                rotation = np.matmul(rot_matrix, rotation)
                rotation = utils3d.numpy.matrix_to_quaternion(rotation)
            elif transform.shape == (3, 3):
                # Legacy 3x3 pure rotation
                xyz = np.matmul(xyz, transform.T)
                rotation = utils3d.numpy.quaternion_to_matrix(rotation)
                rotation = np.matmul(transform, rotation)
                rotation = utils3d.numpy.matrix_to_quaternion(rotation)
            else:
                raise ValueError(f"Transform must be 3x3 or 4x4, got {transform.shape}")
